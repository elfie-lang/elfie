...
impl Binder {
    ...
    fn exec_declaration(&mut self, r: NodeRef, symbol: SymbolId, env: &mut Env) {
        ...
        let clause_holder = trees
            .child(r, E::Signature)
            .or_else(|| trees.child(r, E::Declared))
            .unwrap_or(r);
    ...
    fn apply_clause(&mut self, entity: EntityId, clause: NodeRef, extends: bool, env: &mut Env) {
        let trees = self.trees.clone();
        let Some(uses) = trees.child(clause, E::TraitUses) else {
            return;
        };
    ...
    fn include_members(&mut self, receiver: EntityId, from: EntityId) {
        let Some(from_scope) = self.model.entities[from].scope else {
            return;
        };
    ...
    fn join_member(&mut self, receiver: EntityId, member: SymbolId) {
        let scope = match self.model.entities[receiver].scope {
            Some(scope) => scope,
            None => {
                let owner = self.model.entities[receiver]
                    .node
                    .unwrap_or(super::bind::SYNTHETIC);
        ...
        if let Some(existing) = self.model.lookup_local(scope, &name) {
            // Decision: only a later trait overrides an earlier one; the entity's own
            // member, or one inherited from data, keeps its definition.
            let trees = self.trees.clone();
            let existing_trait =
                trees.ancestor(self.model.symbols[existing].node, S::TraitDeclaration);
    ...
    fn exec_where(&mut self, r: NodeRef, env: &mut Env) {
        let trees = self.trees.clone();
        let Some(conditions) = trees.child(r, S::Conditions) else {
            return;
        };
    ...
    fn exec_with(&mut self, r: NodeRef, env: &mut Env) {
        ...
        let Some(&expression) = children.iter().find(|&&c| !trees.is(c, S::Block)) else {
            return;
        };
    ...
    fn exec_for(&mut self, r: NodeRef, env: &mut Env) {
        let trees = self.trees.clone();
        let Some(block) = trees.child(r, S::Block) else {
            return;
        };
    ...
    fn loop_bindings(
        ...
        let trees = self.trees.clone();
        let declared = trees.child(r, E::Declared)?;
        let mut names = vec![self.declared_name(declared)?];
        let from = trees.child(r, S::ForFrom);
        let (iterables, keys) = match (trees.child(r, S::ForInOf), from) {
            (Some(in_of), _) => (
                trees.child_nodes(in_of),
                trees.has_token(in_of, K::OfKeyword),
            ),
            (None, Some(from)) => {
                let second = trees
                    .child(from, E::Declared)
                    .and_then(|d| self.declared_name(d));
                names.extend(second);
                (
                    trees
                        .child_nodes(from)
                        .into_iter()
                        .filter(|&c| !trees.is(c, E::Declared))
                        .collect(),
                    false,
                )
            }
    ...
    fn declared_name(&self, declared: NodeRef) -> Option<String> {
        let trees = &self.trees;
        trees
            .child_token(
                declared,
            ...
            .map(|i| trees.token(declared.file, i).value.clone())
    }
    ...
    fn iterate(&self, value: Value, keys: bool) -> Vec<Value> {
        match value {
            ...
            Value::Object(pairs) => pairs
                .into_iter()
                .map(|(k, v)| if keys { Value::String(k) } else { v })
                .collect(),
            Value::Entity(entity) => {
    ...
    pub(crate) fn eval(&mut self, r: NodeRef, env: &mut Env) -> Value {
        ...
        if rule == E::Name.entity() {
            let Some(name) = trees.name(r) else {
            ...
            return match self
                ...
                .or_else(|| self.lookup(self.model.enclosing_scope(r), &name))
            {
        ...
        if rule == E::Nullish.entity() {
            return if trees.raw(r).trim() == "null" {
                Value::Null
            } else {
                Value::Undefined
            };
        ...
        if rule == E::Object.entity() {
            ...
            for key in trees.children_of(r, E::ObjectKey) {
                ...
                let value = trees
                    ...
                    .map_or(Value::Undefined, |v| self.eval(v, env));
        ...
        if rule == E::Group.entity() {
            return trees
                ...
                .into_iter()
                .next()
                .map_or(Value::Undefined, |inner| self.eval(inner, env));
        }
        ...
        if rule == E::Dereference.entity() {
            let Some(operand) = trees.child_nodes(r).into_iter().next() else {
                return Value::Undefined;
            };
        ...
        if rule == E::Traits.entity() {
            ...
            let Some(left) = trees.left(r) else {
            ...
            let uses = trees
                .child(r, E::TraitUses)
                .map(|u| trees.children_of(u, E::TraitUse))
                .unwrap_or_default();
        ...
        if rule == E::Definition.entity() {
            return trees
                ...
                .into_iter()
                .next()
                .map_or(Value::Undefined, |left| self.eval(left, env));
        }
        ...
        if rule == E::Index.entity() {
            ...
            return match (left, index) {
                ...
                (Value::Object(pairs), Value::String(key)) => pairs
                    .into_iter()
                    .find(|(k, _)| *k == key)
        ...
        if rule == E::AwaitOperation.entity()
            ...
            || rule == E::FromOperation.entity()
        {
            return trees
                ...
                .into_iter()
                .next()
                .map_or(Value::Undefined, |o| self.eval(o, env));
        }
        Value::Undefined
    }
    ...
    fn eval_infix(&mut self, r: NodeRef, env: &mut Env) -> Value {
        ...
        if rule == E::AdditiveOperation.entity() {
            ...
            return match (left, right) {
                (Value::Number(a), Value::Number(b)) => {
                    Value::Number(if minus { a - b } else { a + b })
                }
                (a, b) if !minus => {
                    Value::String(format!("{}{}", self.to_text(&a), self.to_text(&b)))
                }
                _ => Value::Undefined,
            };
        }
        ...
        if rule == E::MultiplicativeOperation.entity() || rule == E::PowerOperation.entity() {
            if let (Value::Number(a), Value::Number(b)) = (left, right) {
                ...
                let text = operator
                    .map(|i| trees.token(r.file, i).value.clone())
                    .unwrap_or_default();
    ...
    fn eval_assignment(&mut self, r: NodeRef, env: &mut Env) -> Value {
        ...
        if trees.is(left, E::Current) {
            let Some((accessor, Some(name))) = trees.accessor_and_name(left) else {
                return Value::Undefined;
            };
    ...
    fn eval_current(&mut self, r: NodeRef, env: &mut Env) -> Value {
        ...
        self.access(
            ...
            trees.token(r.file, accessor).rule.expect("accessor"),
            name.as_deref(),
            env,
        )
    }
    ...
    fn access(&mut self, value: Value, accessor: Rule, name: Option<&str>, env: &mut Env) -> Value {
        ...
        match layer {
            ...
            Layer::Scope => {
                ...
                match name {
                    None => self.model.entities[entity]
                        .scope
                        .map_or(Value::Undefined, Value::Scope),
            ...
            Layer::Parent => {
                ...
                let scope = self.model.entities[entity]
                    .scope
    ...
    fn context_property(&mut self, value: &Value, name: &str) -> Value {
        match value {
            Value::Entity(entity) => {
                ...
                match ContextProperty::lookup(name) {
                    ...
                    Some(ContextProperty::Type) => self.model.entities[entity]
                        .ty
                        .clone()
                        .map_or(Value::Undefined, |t| Value::Type(Box::new(t))),
                    ...
                    Some(ContextProperty::Entities) => Value::List(
                        self.model.entities[entity]
                            .entities()
                            .iter()
                            .map(|&e| Value::Entity(e))
                            .collect(),
                    ),
                    Some(ContextProperty::Extenders) => Value::List(
                        self.model.entities[entity]
                            .extenders()
                            .iter()
                            .map(|&e| Value::Entity(e))
                    ...
                    Some(ContextProperty::Parameters) => Value::List(
                        self.model.entities[entity]
                            .parameters()
                            .iter()
                            .map(|&s| Value::Entity(self.model.symbols[s].entity))
                            .collect(),
                    ),
            ...
            Value::List(items) => match name {
                "type" => Value::Type(Box::new(TypeRef::List(Box::new(TypeRef::Unknown(
                    String::new(),
                ))))),
                "like" => Value::Like(Box::new(value.clone())),
                "itemType" => items.first().map_or(Value::Undefined, |first| {
                    self.context_property(first, "type")
                }),
                _ => Value::Undefined,
            },
            _ => match name {
        ...
        match self.model.symbols[symbol].kind {
            SymbolKind::Member => {
                ...
                if trees.is(node, E::TypeKey) {
                    return Value::Type(Box::new(
                        self.model.entities[symbol_entity]
                            .ty
                            .clone()
    ...
    fn eval_call(&mut self, r: NodeRef, env: &mut Env) -> Value {
        let trees = self.trees.clone();
        let Some(callee) = trees.left(r) else {
            return Value::Undefined;
        };
        let arguments = trees.arguments(r);
        // Method calls.
        if trees.is(callee, E::Member)
            && let (Some(left), Some((accessor, Some(name)))) =
                (trees.left(callee), trees.accessor_and_name(callee))
            && (trees.token_is(callee.file, accessor, P::ValueAccessor)
                || trees.token_is(callee.file, accessor, P::OptionalValueAccessor))
        {
            ...
            match (&receiver, method.as_str()) {
                ...
                (Value::List(items), "map") => {
                    ...
                    let mapped = items
                        .into_iter()
                        .map(|item| self.call_value(&function, vec![item], env))
                        .collect();
                ...
                (Value::List(items), "join") => {
                    let separator = arguments
                        .first()
                        .map_or(Value::String(",".into()), |&a| self.eval(a, env));
                ...
                (Value::List(items), "includes") => {
                    let needle = arguments
                        .first()
                        .map_or(Value::Undefined, |&a| self.eval(a, env));
        ...
        match function {
            ...
            Value::Like(_) => {
                // `X@like(prompt)`: a value of the entity's type that the prompt
                // describes; at bind time, the prompt itself.
                // @lfy def/model/main.lfy:bind
                arguments
                    .first()
                    .map_or(Value::Undefined, |&a| self.eval(a, env))
    ...
    fn call_value(&mut self, function: &Value, values: Vec<Value>, env: &mut Env) -> Value {
        ...
        match function {
            ...
            Value::Entity(entity) => {
                ...
                self.bind_parameters(node, &values, &mut inner);
                let Some(body) = trees.child(node, S::Block) else {
                    return Value::Undefined;
                };
    ...
    fn bind_parameters(&mut self, function: NodeRef, values: &[Value], env: &mut Env) {
        let trees = self.trees.clone();
        let Some(parameters) = trees.child(function, E::Parameters).or_else(|| {
            trees
                .child(function, E::Signature)
                .and_then(|s| trees.child(s, E::Parameters))
        }) else {
            return;
        };
        let mut rest = values.iter();
        for parameter in trees.child_nodes(parameters) {
            ...
            let Some(name) = trees.child(parameter, E::Name).and_then(|n| trees.name(n)) else {
                continue;
            };
            let value = if spread {
                Value::List(rest.by_ref().cloned().collect())
        ...
        if trees.is(argument, E::Object) {
            for key in trees.children_of(argument, E::ObjectKey) {
                ...
                match name.as_str() {
                    "input" => {
                        test.input = value;
                        test.input_text = value
                            .map(|v| trees.raw(v).trim().to_string())
                            .unwrap_or_default();
                    }
                    "expect" => {
                        test.expect = value;
                        test.expect_text = value
                            .map(|v| trees.raw(v).trim().to_string())
                            .unwrap_or_default();
                    }
                    _ => {}
    ...
    pub(crate) fn to_text(&self, value: &Value) -> String {
        match value {
            ...
            Value::List(items) => items
                .iter()
                .map(|i| self.to_text(i))
                .collect::<Vec<_>>()
                .join(", "),
            Value::Object(pairs) => pairs
                .iter()
                .map(|(k, v)| format!("{k} = {}", self.to_text(v)))
                .collect::<Vec<_>>()
                .join(", "),
            Value::Entity(e) => self.model.entities[*e]
                .identifier
                .clone()
    ...
    pub(crate) fn type_text(&self, ty: &TypeRef) -> String {
        match ty {
            TypeRef::Entity(e) => self.model.entities[*e]
                .identifier
                .clone()
                .unwrap_or_else(|| "anonymous".to_string()),
    ...
    fn reference_text(&mut self, reference: NodeRef, env: &mut Env) -> String {
        ...
        if rule == E::Dereference.entity() {
            let Some(operand) = trees.child_nodes(reference).into_iter().next() else {
                return String::new();
            };
            return self.reference_text(operand, env);
        }
        if rule == E::Current.entity() {
            let Some((accessor, name)) = trees.accessor_and_name(reference) else {
                return trees.raw(reference);
            };
            ...
            return match name {
                ...
                Some(name) => format!(
                    "{head}{}{}",
                    trees.token(reference.file, accessor).raw,
                    trees.token(reference.file, name).value
                ),
            };
        }
        if rule == E::Member.entity() {
            let Some(left) = trees.left(reference) else {
                return trees.raw(reference);
            };
            let Some((accessor, name)) = trees.accessor_and_name(reference) else {
                return trees.raw(reference);
