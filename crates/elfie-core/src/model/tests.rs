...
fn source(path: &str, text: &str, uses: &[Option<&str>]) -> Source {
    let tokens = crate::lexer::lex(text, Some(path)).unwrap_or_else(|error| panic!("{path}: {error}"));
    let tree = crate::parser::parse(tokens, None);
    assert!(tree.errors.is_empty(), "{path} does not parse: {}", tree.render());
    Source { path: path.to_string(), tree, uses: uses.iter().map(|u| u.map(str::to_string)).collect() }
}
...
fn problems(model: &Model) -> Vec<String> {
    model.problems.iter().map(|p| format!("{}: {}", model.raw(p.node).trim(), p.message)).collect()
}
...
fn names(model: &Model, scope: ScopeId) -> Vec<String> {
    model.scopes[scope].symbols.iter().map(|&s| model.symbols[s].name.clone()).collect()
}
...
fn node<R: GrammarRule + Copy>(model: &Model, file: FileId, rule: R, raw: &str) -> NodeRef {
    nodes(model, file, rule, raw)
        .first()
        .copied()
        .unwrap_or_else(|| panic!("no {} node reading {raw:?} in file {file}", rule.entity().identifier()))
}

/// The usage a node made.
fn usage(model: &Model, r: NodeRef) -> &Usage {
    let id = model.usage_of(r).unwrap_or_else(|| panic!("no usage for {:?}", model.raw(r)));
    &model.usages[id]
}
...
fn member(model: &Model, entity: EntityId, name: &str) -> SymbolId {
    model
        ...
        .unwrap_or_else(|| panic!("entity has no member {name}: {:?}", names(model, model.entities[entity].scope.unwrap())))
}
...
fn test_trait_member_and_application() {
    ...
    assert!(matches!(model.entities[a].traits[0].source, AppliedSource::Is(_)));
    ...
    assert_eq!(model.entities[x_entity].ty, Some(TypeRef::Primitive("string")));
...
fn test_extends_entities_and_extenders() {
    ...
    assert_clean(&model);
    let (a, b, c) = (entity(&model, "a"), entity(&model, "b"), entity(&model, "C"));
    ...
    assert!(matches!(model.entities[c].traits[1].source, AppliedSource::Inherited(0)));
    assert!(matches!(model.entities[b].traits[0].source, AppliedSource::Extends(_)));
}
...
fn block_scope_is_child_of_declaration_scope() {
    ...
    let a_block = model.scope_of(node(&model, 0, S::Block, "{ const inner = 1; }")).expect("a Block owns a scope");
    ...
    let f_block = model.scope_of(node(&model, 0, S::Block, "{ const q = p; }")).unwrap();
...
fn object_key_declares_only_in_an_enum() {
    ...
    assert_eq!(names(&model, model.entities[color].scope.unwrap()), ["red", "blue"]);
    for m in &members {
        assert_eq!(model.symbols[*m].kind, SymbolKind::EnumMember);
        assert_eq!(model.entities[model.symbols[*m].entity].kind, EntityKind::EnumMember);
    }
...
fn member_statement_declares_a_member() {
    ...
    assert_eq!(model.entities[ys].ty, Some(TypeRef::List(Box::new(TypeRef::Primitive("number")))));
}
...
fn scope_current_entity() {
    ...
    assert_clean(&model);
    let a = entity(&model, "A");
    let with = model.scope_of(node(&model, 0, S::With, "with A { const x = 1; }")).expect("a With owns a scope");
    assert_eq!(model.scopes[with].current, a);
    let with_block = model.scope_of(node(&model, 0, S::Block, "{ const x = 1; }")).unwrap();
    assert_eq!(model.scopes[with_block].parent, Some(with));
    assert_eq!(model.scopes[with_block].current, a);
    let loop_scope = model.scope_of(node(&model, 0, S::For, "for (const i in [1]) { const j = i; }")).unwrap();
    assert_eq!(model.scopes[loop_scope].current, model.file_entities[0]);
    let loop_block = model.scope_of(node(&model, 0, S::Block, "{ const j = i; }")).unwrap();
...
fn use_without_as_imports_own_symbols_only() {
    ...
    let a = source("a.lfy", "use \"./b\"; const x = B; const y = C;", &[Some("b.lfy")]);
    ...
    assert_eq!(usage(&model, node(&model, a_file, E::Name, "B")).symbol, Some(b_symbol));
    ...
    // b.lfy itself sees C.
    assert_eq!(model.lookup(model.file_scopes[b_file], "C"), Some(file_symbol(&model, model.file("c.lfy").unwrap(), "C")));
}
...
fn use_with_as_resolves_members_in_the_module() {
    ...
    let a = source("a.lfy", "use \"./b\" as M; const x = M.B; d A is M.tb {} for (const e in M) {}", &[Some("b.lfy")]);
    ...
    assert_eq!(entities_of(&model, model.symbols[file_symbol(&model, b_file, "tb")].entity), vec![a]);
    // A For over the module visits its symbols' entities.
    let e = model.lookup_local(model.scope_of(node(&model, a_file, S::For, "for (const e in M) {}")).unwrap(), "e").unwrap();
...
fn apply_call_applies_the_trait() {
    ...
    assert_eq!(applied.source, AppliedSource::Apply(node(&model, 0, E::Call, "X.apply(Y, 3)")));
    assert_eq!(entities_of(&model, x), vec![y]);
    // The alternationList: every item, not the list itself.
    let (marker, alt, p, q) = (entity(&model, "marker"), entity(&model, "Alt"), entity(&model, "P"), entity(&model, "Q"));
...
fn extends_chain_applies_every_base() {
    ...
    assert_clean(&model);
    let (a, b, c) = (entity(&model, "a"), entity(&model, "b"), entity(&model, "C"));
    assert_eq!(model.entities[c].value("v"), Some(&Value::String("xmy".to_string())));
    let traits: Vec<(EntityId, Vec<Value>)> = model.entities[c].traits.iter().map(|t| (t.entity, t.values.clone())).collect();
    assert_eq!(traits, [(b, vec![Value::String("m".into())]), (a, vec![Value::String("xmy".into())])]);
    assert_eq!(model.entities[c].traits[1].source, AppliedSource::Inherited(0));
    assert_eq!(entities_of(&model, a), vec![c]);
    assert_eq!(model.entities[a].extenders(), &[b]);
}
...
fn applied_trait_body_lands_on_the_receiver() {
    ...
    assert_eq!(model.symbols[m].entity, model.symbols[member(&model, t, "m")].entity);
...
fn two_traits_declaring_the_same_member_is_a_problem() {
    ...
    assert_eq!(model.problems.len(), 1, "{:?}", problems(&model));
    assert!(model.problems[0].message.contains('m'), "{}", model.problems[0].message);
    let (t1, t2, a) = (entity(&model, "t1"), entity(&model, "t2"), entity(&model, "A"));
    assert_eq!(names(&model, model.entities[a].scope.unwrap()), ["m"]);
    let m = model.symbols[member(&model, a, "m")].entity;
    assert_eq!(m, model.symbols[member(&model, t1, "m")].entity);
    assert_ne!(m, model.symbols[member(&model, t2, "m")].entity);
    assert_eq!(model.entities[m].definition.as_deref(), Some("first"));
    assert_eq!(model.entities[a].traits.iter().map(|t| t.entity).collect::<Vec<_>>(), [t1, t2]);
}
...
fn trait_entities_in_file_then_application_order() {
    ...
    let a = source("a.lfy", "use \"./b\"; d A is t {} t.apply(A);", &[Some("b.lfy")]);
    let model = bind(vec![b, a]);
    assert_clean(&model);
...
fn previous_yields_the_earlier_declaration() {
    ...
    let inner = model.usages.iter().filter(|u| u.layer == Layer::Previous).map(|u| u.symbol).collect::<Vec<_>>();
    assert_eq!(inner, [Some(y), None]);
    assert_eq!(model.problems.len(), 1, "{:?}", problems(&model));
    assert_eq!(model.info(model.problems[0].node).rule, E::Previous.entity());
}
...
fn template_reference_resolves_in_scope_or_to_the_rule() {
    ...
    let other = source("a.lfy", "d X {} const y = `[[X]] and [[Foo]]`; const z = Foo;", &[]);
    ...
    let names_in_a: Vec<NodeRef> = (0..model.nodes[a_file].len())
        .map(|index| NodeRef { file: a_file, index })
        .filter(|&r| model.info(r).rule == E::Name.entity())
        .collect();
    let resolved: Vec<(String, Option<SymbolId>)> = names_in_a
        .iter()
        .filter_map(|&r| model.usage_of(r).map(|u| (model.raw(r).trim().to_string(), model.usages[u].symbol)))
        .collect();
    assert_eq!(resolved, [("X".to_string(), Some(x)), ("Foo".to_string(), Some(foo)), ("Foo".to_string(), None)]);
...
fn unknown_context_property_is_a_problem() {
    ...
    assert_eq!(model.problems[0].message, "nonsense is not a context property");
    ...
    assert_eq!(usage(&model, node(&model, 0, E::Current, "@identifier")).layer, Layer::Context);
    assert_eq!(usage(&model, node(&model, 0, E::Member, "X@type")).layer, Layer::Context);
}
...
fn member_on_data_resolves_to_its_member() {
    ...
    assert_eq!((value.layer, value.name.as_deref(), value.symbol), (Layer::Value, Some("m"), Some(m)));
    let scope = usage(&model, node(&model, 0, E::Member, "D$m"));
    assert_eq!((scope.layer, scope.name.as_deref(), scope.symbol), (Layer::Scope, Some("m"), Some(m)));
...
fn name_resolves_to_the_nearest_scope() {
    ...
    assert_clean(&model);
    let f = entity(&model, "f");
    let parameter = model.lookup_local(model.entities[f].scope.unwrap(), "n").unwrap();
    let global = file_symbol(&model, 0, "n");
    let uses: Vec<Option<SymbolId>> = model.usages.iter().filter(|u| u.name.as_deref() == Some("n")).map(|u| u.symbol).collect();
...
fn identifier_definition_parameters_and_output() {
    ...
    assert_eq!(model.entities[entity(&model, "B")].definition.as_deref(), Some("about B"));
    let f = &model.entities[entity(&model, "f")];
    assert_eq!(f.identifier.as_deref(), Some("f"));
    assert_eq!(f.definition.as_deref(), Some("does f"));
    let parameters: Vec<(String, SymbolKind)> = f.parameters().iter().map(|&s| (model.symbols[s].name.clone(), model.symbols[s].kind)).collect();
    assert_eq!(parameters, [("a".to_string(), SymbolKind::Parameter), ("rest".to_string(), SymbolKind::Parameter)]);
    ...
    assert_eq!(model.entities[rest].ty, Some(TypeRef::List(Box::new(TypeRef::Primitive("number")))));
}
...
fn type_and_item_type() {
    ...
    assert_eq!(s.ty, Some(TypeRef::Literal(Box::new(Value::String("text".into())))));
    let d = entity(&model, "D");
    let items = &model.entities[model.symbols[member(&model, d, "items")].entity];
    assert_eq!(items.ty, Some(TypeRef::List(Box::new(TypeRef::Primitive("string")))));
...
fn add_and_where_append_criteria() {
    ...
    assert_eq!(criteria[0].behavior, Some(vec!["b1".to_string(), "b2".to_string()]));
    ...
    assert_eq!(criteria[2].situation, Some(vec!["w1 or w2".to_string(), "w3".to_string()]));
    assert_eq!(criteria[2].behavior, Some(vec!["then".to_string()]));
    assert_eq!(criteria[2].contributor, a);
    let b = entity(&model, "B");
    let in_with: Vec<Option<Vec<String>>> = model.entities[b].acceptance_criteria.iter().map(|c| c.behavior.clone()).collect();
    assert_eq!(in_with, [Some(vec!["from with".to_string()]), Some(vec!["w".to_string()])]);
    assert!(model.entities[model.file_entities[0]].acceptance_criteria.is_empty());
}
...
fn test_call_appends_tests() {
    ...
    assert_eq!(tests[0].input.map(|r| model.info(r).rule), Some(E::Number.entity()));
...
fn criteria_of_strips_references() {
    ...
    assert_eq!(raw.behavior, Some(vec!["see [[B]] and [[B.x]]".to_string()]));
    let resolved = criteria_of(&model, a);
    assert_eq!(resolved.len(), 1);
    assert_eq!(resolved[0].situation, Some(vec!["given B".to_string()]));
    assert_eq!(resolved[0].behavior, Some(vec!["see B and B.x".to_string()]));
...
fn find_entity(model: &Model, name: &str, keep: impl Fn(&Entity) -> bool) -> EntityId {
    let found: Vec<EntityId> = (0..model.entities.len())
        .filter(|&e| model.entities[e].identifier.as_deref() == Some(name) && keep(&model.entities[e]))
        .collect();
...
fn object_field<'a>(value: &'a Value, key: &str) -> &'a Value {
    match value {
        Value::Object(pairs) => &pairs.iter().find(|(k, _)| k == key).unwrap_or_else(|| panic!("no field {key}")).1,
...
fn repository_binds_without_problems() {
    ...
    let rule = model.trait_named("rule").expect("the grammar declares rule");
    ...
    assert_eq!(object_field(space_rule, "identifier"), &Value::String("Space".into()));
    let text = object_field(space_rule, "text").as_str().unwrap();
    assert!(text.starts_with("Space = \" \" | \"\t\" | "), "{text:?}");
    assert!(text.ends_with("| \"\u{2028}\" | \"\u{2029}\" ;"), "{text:?}");
    ...
    assert!(model.entities[lex].acceptance_criteria.len() > 200, "{}", model.entities[lex].acceptance_criteria.len());
    assert_eq!(criteria_of(model, lex).len(), model.entities[lex].acceptance_criteria.len());
    // NegateOperation: two binding applications.
    let binding = model.trait_named("binding").expect("the grammar declares binding");
    let negate = find_entity(model, "NegateOperation", |e| e.kind == EntityKind::Data);
    let bindings = model.entities[negate].traits.iter().filter(|a| a.entity == binding).count();
    ...
    let syntax = object_field(model.entities[primary].value("rule").unwrap(), "syntax").as_str().unwrap();
    assert!(syntax.starts_with("[[StringLiteral]] | [[Template]] | [[Number]] | "), "{syntax:?}");
